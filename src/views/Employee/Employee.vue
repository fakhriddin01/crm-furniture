...
const activate = (id) => {
    useEmployee.activate(id).then((res)=>{
        if(res.status == 201){
            toast.info('staff is active');
            listUpdate();
        }
        else{
...
const deactivate = (id) => {
    useEmployee.deactivate(id).then((res)=>{
        if(res.status == 201){
            toast.info('staff is deactiveted');
            listUpdate();
        }else{
            toast.info(res.message)
        }
...
const filterSearch = () =>{
    if(search.input == ''){
        listUpdate();
            ...
            useEmployee.listByName(search.input).then((res)=>{
                store.state.list = res.data.staffs
                pagination.total_page = res.data.total_pages
                pagination.current_page = res.data.current_page
                pagination.total_number = res.data.allStaff
...
const addEmployee=()=>{
    ...
    useEmployee.create(employee).then((res)=>{
        if(res.status == 201){
            toast.success('successfully added employee !', {autoClose: 500, theme: 'dark', pauseOnHover: false})
            employeeInfo.fullname = ''
            employeeInfo.phone_number = ''
            employeeInfo.role = ''
            employeeInfo.card_number = ''
            employeeInfo.login = ''
...
<template>
  ...
  <div class="p-3">
    <h2 class="uppercase dark:text-white text-gray-900">Hodimlar</h2>
            ...
            <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                ...
                <form  @submit.prevent="addEmployee">
                    <div class="grid gap-4 mb-4 sm:grid-cols-2">
                        <div>
                            <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Ism Sharif</label>
                            <input v-model="employeeInfo.fullname" type="text" name="name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Ism Sharifni kiriting" required="">
                        </div>
                        <div>
                            <label for="brand" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Tel:</label>
                            <input v-model="employeeInfo.phone_number" type="tel" name="brand" id="brand" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Telefon raqam" required="">
                        </div>
                        <div>
                            <label for="price" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Karta raqami</label>
                            <input v-model="employeeInfo.card_number" type="text" name="price" id="price" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Plastik karta raqami" required="">
                        </div>
                        <div>
                            <label for="category" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Lavozim</label>
                            <select v-model="employeeInfo.role" id="category" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        ...
                        <div>
                            <label for="brand" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Login</label>
                            <input v-model="employeeInfo.login" type="text" name="brand" id="brand" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Login kiriting" required="">
                        </div>
                        <div>
                            <label for="brand" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Parol</label>
                            <input v-model="employeeInfo.password" type="password" name="brand" id="brand" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Parolni kiriting" required="">
                        </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            <th scope="col" class="px-4 py-3">I.SH.O</th>
                    ...
                    <tbody>
                        <tr v-for="(el, index) in LIST" :key="index" class="border-b dark:border-gray-700">
                            <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ el.fullname }}</th>
                            <td class="px-4 py-3">{{ el.phone_number }}</td>
                            <td class="px-4 py-3">{{el.role}}</td>
                            <td class="px-4 py-3">{{ el.card_number }}</td>
                            <td class="px-4 py-3">{{ el.is_active ? "Active" : "NotActive" }}</td>
