...
const listUpdate = () => {
    useProduct.list().then((res)=>{
        console.log(res.data.products[5]);
...
const addProduct= async ()=>{
    ...
    useProduct.create(product).then((res)=>{
        if(res.status == 201){
...
onMounted(()=>{
    useCategory.list().then((res)=>{
        ...
        else if(error.message == 'token expired' || error.message == 'token not found'){
            router.push({name: 'login'})
            toast.error('Token expired');
        }
        else{
                ...
                <form @submit.prevent="addProduct">
                    <div class="grid gap-4 mb-4 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input type="file" accept=".jpg , .png" name="brand" id="brand" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Mahsulot rasmini kiriting" >
                        </div>
                        <div> 
                        ...
                        <div>
                            ...
                            <select  v-model="productInfo.type" id="category" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                ...
                                <option v-for="el in TYPE_LIST" :key="el.index">{{ el.name }}</option>
                            </select>
                        </div>
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        <tr v-for="(el, index) in LIST" :key="el.index" class="border-b dark:border-gray-700">
                            ...
                            <td class="px-4 py-3"><img :src="el.file" class="rounded-full object-cover w-[42px] h-[42px] border border-dashed" alt=""></td>
