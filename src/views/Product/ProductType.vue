...
const productTypeInfo = reactive({
    name: '',
})
...
const addProductType=()=>{
    ...
    useCategory.create(productType).then((res)=>{
        if(res.status == 201){
        ...
        if(error.message == 'token expired' || error.message == 'token not found1'){
            router.push({name: 'login'})
        }
        console.log(error.message);
        toast.error();
...
const deactivate = (id) => {
    useCategory.remove(id).then((res)=>{
        if(res.status == 200){
...
<template>
  ...
  <div class="p-3">
    <h2 class="uppercase dark:text-white text-gray-900">Mahsulotlar turi</h2>
            ...
            <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                ...
                <form @submit.prevent="addProductType">
                    <div class="grid gap-4 mb-4 sm:grid-cols-1">
                        <div>
                            <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Mahsulot turi</label>
                            <input v-model="productTypeInfo.name" type="text" name="name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Mahsulot turini kiriting" required="">
                        </div>
                    </div>
                ...
                <div class="w-full md:w-1/2">
                    <form class="flex items-center">
                        ...
                        <div class="relative w-full">
                            ...
                            <input v-model="searchText" type="text" id="simple-search" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Qidiruv" required="">
                        </div>
                    </form>
                </div>
                            ...
                            <td @mouseleave="cancelToggel" class="px-4 py-3 flex flex-col items-center justify-center">
                                ...
                                <div v-if="action == el._id" class="z-10 absolute bottom-8 w-22 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600" >
                                    <ul class="py-0 text-sm text-gray-700 dark:text-gray-200">
                                        ...
                                        <li>
                                            <a @click="deactivate(el._id)" class="block py-1 px-4 text-red-500 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Delete</a>
                                        </li>
                                    </ul>
                                </div>
                            </td>
